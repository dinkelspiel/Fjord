...
namespace Proj.Game {
    public class tilemap_editor : scene {
        ...
        public override void update() {
            ...
            if(mouse.button_pressed(0) && selected_tile != -1 && math_uti.mouse_inside(grid_x, grid_y, grid_x_end, grid_y_end)) {
                var x_ = (mouse.x - grid_x);
                var y_ = (mouse.y - grid_y);
                x_ = x_ / Tilemap.grid_w * Tilemap.grid_w;
                y_ = y_ / Tilemap.grid_h * Tilemap.grid_h;
                x_ = x_ / 16;
                y_ = y_ / 16;
                x_ = x_ / 2;
                y_ = y_ / 2;
        ...
        public override void render() {
            for(var i = 0; i < Tilemap.w; i++) {
                for(var j = 0; j < Tilemap.h; j++) {
                    SDL.SDL_Rect rect;
                    rect.x = i * (int)(Tilemap.grid_w * zoom) - (int)camera.camera_position.X;
                    rect.y = j * (int)(Tilemap.grid_h * zoom) - (int)camera.camera_position.Y;
                    ...
                    if(Tilemap.map[i, j] == 1) {
                        ...
                        draw.texture_ext(game_manager.renderer, texs[0], rect.x + w / 2, rect.y + h / 2, 0, w, h);
                    }
                }
            }
